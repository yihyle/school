...
import type { MyCourseResponse } from '@/types';
      ...
      {error ? (
        ...
        <>
          <p className="text-sm text-[#717171] mb-6">
            총 <span className="font-semibold text-[#222222]">{courses.length}</span>개의 강의 수강 중
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {courses.map((course) => {
              const gradient = stringToColor(course.courseTitle);
              const progressPercent = course.progressRate ?? 0;

              return (
                <div key={course.enrollmentId} className="bg-white rounded-2xl overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-200 group">
                  <div className={`relative aspect-video bg-gradient-to-br ${gradient} rounded-2xl overflow-hidden`}>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-3xl font-bold text-white/60">{course.courseTitle.charAt(0)}</span>
                    </div>
                    {course.completed && (
                  ...
                  <div className="pt-3 pb-1 px-1">
                    ...
                    <h3 className="font-semibold text-[#222222] text-sm leading-snug mb-1 line-clamp-2">{course.courseTitle}</h3>
